...
char* ReadOffset(unsigned char* sector, const char* offset, int numBytes, bool reverse);
void WriteOffset(unsigned char* sector, const char* offset, const char* description, bool reverse);
...
char* ReadFat(int cluster)
{
	...
	res = ReadOffset(sector, Dec2Hex(cluster % (byteSector / OFFSET_CLUS) * OFFSET_CLUS), OFFSET_CLUS, true);
	return res;
}
...
unsigned char* WriteFat(int cluster, const char* description)
{
	...
	WriteOffset(sector, Dec2Hex(cluster % (byteSector / OFFSET_CLUS) * OFFSET_CLUS), description, true);
	return sector;
}
...
int main()
{
	...
	byteSector = Hex2Dec((char*)ReadOffset(BootSector, "B", 2, true));
	sectorClus = Hex2Dec((char*)ReadOffset(BootSector, "D", 1, true));
	sectorBoot = Hex2Dec((char*)ReadOffset(BootSector, "E", 2, true));
	numFat = Hex2Dec((char*)ReadOffset(BootSector, "10", 1, true));
	numEntry = Hex2Dec((char*)ReadOffset(BootSector, "11", 2, true));
	sectorVol = Hex2Dec((char*)ReadOffset(BootSector, "20", 4, true));
	sectorFat = Hex2Dec((char*)ReadOffset(BootSector, "24", 4, true));
	RDET = Hex2Dec((char*)ReadOffset(BootSector, "2C", 4, true));
	...
	while (1)
	{
		for (int indexSector = sectorEntry; indexSector < sectorEntry + sectorClus; indexSector++)
		{
			...
			for (int offset = 0; offset < SEC_SIZE; offset += RETRY_SIZE)
			{
				...
				{
					printf("Recovering file: %c\n", loading);
					loading = Loading(loading);
					WriteOffset(Entry, Dec2Hex(offset), "46", true);
					...
					int startCluster = Hex2Dec(ReadOffset(Entry, Dec2Hex(offset + 26), 2, true));
	...
	printf("Recover file successfully!\n");
	return 0;
}